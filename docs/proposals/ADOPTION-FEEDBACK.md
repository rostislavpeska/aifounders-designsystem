...
| 2026-07-07 | AIF | tertiary button on light-2/light-3 sections (homepage editorial, archives) | **F3 — NEEDS VERDICT** | `.btn--tertiary` renders identically on light-1/2/3 (gray-300 border, transparent bg) — confirmed in the DS styleguide surfaces tab, so the theme is CORRECT/matches DS. Operator wants it to visually POP on the gray sections. The DS deliberately doesn't transform tertiary on light surfaces (only dark/brand/support, for readability). | **PARKED for operator ruling.** Recommended DS change if approved: on `.content-section--secondary/--tertiary`, give `.btn--tertiary` a `--raised` (white) resting bg so it lifts off the gray (the `--raised` role is already defined = white on those scopes). One-token DS change; specimen + gate. |
